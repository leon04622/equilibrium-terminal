...
const MEMORY_KEY = "eq-execution-bridge-memory-v1";

export interface ExecutionLessonBridgeMemory {
...
const DEFAULT: ExecutionLessonBridgeMemory = {
...
function loadMemory(): ExecutionLessonBridgeMemory {
  if (typeof window === "undefined") return { ...DEFAULT };
  try {
    const raw = localStorage.getItem(MEMORY_KEY);
    if (!raw) return { ...DEFAULT };
    const p = JSON.parse(raw) as Partial<ExecutionLessonBridgeMemory>;
    return {
...
function saveMemory(m: ExecutionLessonBridgeMemory): void {
...
export interface ExecutionLessonBridgeState {
  active: boolean;
  runId: number;
  step: number;
  memory: ExecutionLessonBridgeMemory;
  recognized: string[];
...
export const useExecutionLessonBridgeStore = create<ExecutionLessonBridgeState>((set, get) => ({
  active: false,
  runId: 0,
  step: 0,
  memory: loadMemory(),
  ...
  markBridgeCompleted: () => {
    const memory = {
      ...
      conceptsMastered: Array.from(
        new Set([...get().memory.conceptsMastered, "execution-certified"]),
      ).slice(0, 50),
    };
    saveMemory(memory);
    set({ memory });
  ...
  markRecognized: (id) => {
    ...
    const checks = ["identify-spread", "identify-limit-order", "identify-size", "identify-exec-pipe"];
    if (checks.every((c) => recognized.includes(c)) && !recognized.includes("execution-pretrade-ready")) {
      recognized = [...recognized, "execution-pretrade-ready"];
    }
    const mastered = new Set(s.memory.conceptsMastered);
    mastered.add(id);
    if (recognized.includes("execution-pretrade-ready")) mastered.add("execution-pretrade-ready");
