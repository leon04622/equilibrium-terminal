...
const MEMORY_KEY = "eq-market-memory-bridge-memory-v1";

export interface MarketMemoryBridgeMemory {
...
const DEFAULT: MarketMemoryBridgeMemory = {
...
function loadMemory(): MarketMemoryBridgeMemory {
  if (typeof window === "undefined") return { ...DEFAULT };
  try {
    const raw = localStorage.getItem(MEMORY_KEY);
    if (!raw) return { ...DEFAULT };
    const p = JSON.parse(raw) as Partial<MarketMemoryBridgeMemory>;
    return {
...
function saveMemory(m: MarketMemoryBridgeMemory): void {
...
export interface MarketMemoryBridgeState {
  active: boolean;
  runId: number;
  step: number;
  memory: MarketMemoryBridgeMemory;
  recognized: string[];
...
export const useMarketMemoryBridgeStore = create<MarketMemoryBridgeState>((set, get) => ({
  active: false,
  runId: 0,
  step: 0,
  memory: loadMemory(),
  ...
  markBridgeCompleted: () => {
    const memory = {
      ...
      certified: true,
      conceptsMastered: Array.from(new Set([...get().memory.conceptsMastered, "market-memory-certified"])).slice(0, 50),
    };
    saveMemory(memory);
    set({ memory });
  },
  ...
  markRecognized: (id) => {
    ...
    if (RECOGNIZE_CHECKS.every((c) => recognized.includes(c)) && !recognized.includes("market-memory-workflow-ready")) {
      recognized = [...recognized, "market-memory-workflow-ready"];
    }
    const mastered = new Set(s.memory.conceptsMastered);
    mastered.add(id);
    if (recognized.includes("market-memory-workflow-ready")) mastered.add("market-memory-workflow-ready");
