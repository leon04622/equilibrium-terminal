...
const MEMORY_KEY = "eq-macro-flows-bridge-memory-v1";

export interface MacroFlowsBridgeMemory {
...
const DEFAULT: MacroFlowsBridgeMemory = {
...
function loadMemory(): MacroFlowsBridgeMemory {
  if (typeof window === "undefined") return { ...DEFAULT };
  try {
    const raw = localStorage.getItem(MEMORY_KEY);
    if (!raw) return { ...DEFAULT };
    const p = JSON.parse(raw) as Partial<MacroFlowsBridgeMemory>;
    return {
...
function saveMemory(m: MacroFlowsBridgeMemory): void {
...
export interface MacroFlowsBridgeState {
  active: boolean;
  runId: number;
  step: number;
  memory: MacroFlowsBridgeMemory;
  recognized: string[];
...
export const useMacroFlowsBridgeStore = create<MacroFlowsBridgeState>((set, get) => ({
  active: false,
  runId: 0,
  step: 0,
  memory: loadMemory(),
  ...
  markBridgeCompleted: () => {
    const memory = {
      ...
      conceptsMastered: Array.from(
        new Set([...get().memory.conceptsMastered, "macro-flows-certified"]),
      ).slice(0, 50),
    };
    saveMemory(memory);
    set({ memory });
