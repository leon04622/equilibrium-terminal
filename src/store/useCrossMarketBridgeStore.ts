...
const MEMORY_KEY = "eq-cross-market-bridge-memory-v1";

export interface CrossMarketBridgeMemory {
...
const DEFAULT: CrossMarketBridgeMemory = {
...
function loadMemory(): CrossMarketBridgeMemory {
  if (typeof window === "undefined") return { ...DEFAULT };
  try {
    const raw = localStorage.getItem(MEMORY_KEY);
    if (!raw) return { ...DEFAULT };
    const p = JSON.parse(raw) as Partial<CrossMarketBridgeMemory>;
    return {
...
function saveMemory(m: CrossMarketBridgeMemory): void {
...
export interface CrossMarketBridgeState {
  active: boolean;
  runId: number;
  step: number;
  memory: CrossMarketBridgeMemory;
  recognized: string[];
...
export const useCrossMarketBridgeStore = create<CrossMarketBridgeState>((set, get) => ({
  active: false,
  runId: 0,
  step: 0,
  memory: loadMemory(),
  ...
  markBridgeCompleted: () => {
    const memory = {
      ...
      conceptsMastered: Array.from(
        new Set([...get().memory.conceptsMastered, "cross-market-certified"]),
      ).slice(0, 50),
    };
    saveMemory(memory);
    set({ memory });
