...
const MEMORY_KEY = "eq-intelligence-desk-bridge-memory-v1";

export interface IntelligenceDeskBridgeMemory {
...
const DEFAULT: IntelligenceDeskBridgeMemory = {
...
function loadMemory(): IntelligenceDeskBridgeMemory {
  if (typeof window === "undefined") return { ...DEFAULT };
  try {
    const raw = localStorage.getItem(MEMORY_KEY);
    if (!raw) return { ...DEFAULT };
    const p = JSON.parse(raw) as Partial<IntelligenceDeskBridgeMemory>;
    return {
...
function saveMemory(m: IntelligenceDeskBridgeMemory): void {
...
export interface IntelligenceDeskBridgeState {
  active: boolean;
  runId: number;
  step: number;
  memory: IntelligenceDeskBridgeMemory;
  recognized: string[];
...
export const useIntelligenceDeskBridgeStore = create<IntelligenceDeskBridgeState>((set, get) => ({
  active: false,
  runId: 0,
  step: 0,
  memory: loadMemory(),
  ...
  markBridgeCompleted: () => {
    const memory = {
      ...
      conceptsMastered: Array.from(
        new Set([...get().memory.conceptsMastered, "intelligence-desk-certified"]),
      ).slice(0, 50),
    };
    saveMemory(memory);
    set({ memory });
