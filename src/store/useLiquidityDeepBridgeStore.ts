...
const MEMORY_KEY = "eq-liquidity-deep-bridge-memory-v1";

export interface LiquidityDeepBridgeMemory {
...
const DEFAULT: LiquidityDeepBridgeMemory = {
...
function loadMemory(): LiquidityDeepBridgeMemory {
  if (typeof window === "undefined") return { ...DEFAULT };
  try {
    const raw = localStorage.getItem(MEMORY_KEY);
    if (!raw) return { ...DEFAULT };
    const p = JSON.parse(raw) as Partial<LiquidityDeepBridgeMemory>;
    return {
...
function saveMemory(m: LiquidityDeepBridgeMemory): void {
...
export interface LiquidityDeepBridgeState {
  active: boolean;
  runId: number;
  step: number;
  memory: LiquidityDeepBridgeMemory;
  recognized: string[];
...
export const useLiquidityDeepBridgeStore = create<LiquidityDeepBridgeState>((set, get) => ({
  active: false,
  runId: 0,
  step: 0,
  memory: loadMemory(),
  ...
  markBridgeCompleted: () => {
    const memory = {
      ...
      conceptsMastered: Array.from(
        new Set([...get().memory.conceptsMastered, "liquidity-certified"]),
      ).slice(0, 50),
    };
    saveMemory(memory);
    set({ memory });
