...
const MEMORY_KEY = "eq-portfolio-risk-bridge-memory-v1";

export interface PortfolioRiskBridgeMemory {
...
const DEFAULT: PortfolioRiskBridgeMemory = {
...
function loadMemory(): PortfolioRiskBridgeMemory {
  if (typeof window === "undefined") return { ...DEFAULT };
  try {
    const raw = localStorage.getItem(MEMORY_KEY);
    if (!raw) return { ...DEFAULT };
    const p = JSON.parse(raw) as Partial<PortfolioRiskBridgeMemory>;
    return {
...
function saveMemory(m: PortfolioRiskBridgeMemory): void {
...
export interface PortfolioRiskBridgeState {
  active: boolean;
  runId: number;
  step: number;
  memory: PortfolioRiskBridgeMemory;
  recognized: string[];
...
export const usePortfolioRiskBridgeStore = create<PortfolioRiskBridgeState>((set, get) => ({
  active: false,
  runId: 0,
  step: 0,
  memory: loadMemory(),
  ...
  markBridgeCompleted: () => {
    const memory = {
      ...
      conceptsMastered: Array.from(
        new Set([...get().memory.conceptsMastered, "portfolio-risk-certified"]),
      ).slice(0, 50),
    };
    saveMemory(memory);
    set({ memory });
  ...
  markRecognized: (id) => {
    ...
    const checks = ["identify-concentration", "identify-correlation", "identify-exposure", "identify-positions"];
    if (checks.every((c) => recognized.includes(c)) && !recognized.includes("portfolio-risk-pretrade-ready")) {
      recognized = [...recognized, "portfolio-risk-pretrade-ready"];
    }
    const mastered = new Set(s.memory.conceptsMastered);
    mastered.add(id);
    if (recognized.includes("portfolio-risk-pretrade-ready")) mastered.add("portfolio-risk-pretrade-ready");
