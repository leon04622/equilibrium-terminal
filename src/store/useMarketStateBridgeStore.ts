...
const MEMORY_KEY = "eq-market-state-bridge-memory-v1";

export interface MarketStateBridgeMemory {
...
const DEFAULT: MarketStateBridgeMemory = {
...
function loadMemory(): MarketStateBridgeMemory {
  if (typeof window === "undefined") return { ...DEFAULT };
  try {
    const raw = localStorage.getItem(MEMORY_KEY);
    if (!raw) return { ...DEFAULT };
    const p = JSON.parse(raw) as Partial<MarketStateBridgeMemory>;
    return {
...
function saveMemory(m: MarketStateBridgeMemory): void {
...
export interface MarketStateBridgeState {
  active: boolean;
  runId: number;
  step: number;
  memory: MarketStateBridgeMemory;
  recognized: string[];
...
export const useMarketStateBridgeStore = create<MarketStateBridgeState>((set, get) => ({
  active: false,
  runId: 0,
  step: 0,
  memory: loadMemory(),
  ...
  markBridgeCompleted: () => {
    const memory = {
      ...
      certified: true,
      conceptsMastered: Array.from(new Set([...get().memory.conceptsMastered, "market-state-certified"])).slice(0, 50),
    };
    saveMemory(memory);
    set({ memory });
  },
  ...
  markRecognized: (id) => {
    ...
    if (RECOGNIZE_CHECKS.every((c) => recognized.includes(c)) && !recognized.includes("market-state-workflow-ready")) {
      recognized = [...recognized, "market-state-workflow-ready"];
    }
    const mastered = new Set(s.memory.conceptsMastered);
    mastered.add(id);
    if (recognized.includes("market-state-workflow-ready")) mastered.add("market-state-workflow-ready");
