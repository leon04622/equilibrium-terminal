...
const MEMORY_KEY = "eq-trade-types-bridge-memory-v1";

export interface TradeTypesBridgeMemory {
...
const DEFAULT: TradeTypesBridgeMemory = {
...
function loadMemory(): TradeTypesBridgeMemory {
  if (typeof window === "undefined") return { ...DEFAULT };
  try {
    const raw = localStorage.getItem(MEMORY_KEY);
    if (!raw) return { ...DEFAULT };
    const p = JSON.parse(raw) as Partial<TradeTypesBridgeMemory>;
    return {
...
function saveMemory(m: TradeTypesBridgeMemory): void {
...
export interface TradeTypesBridgeState {
  active: boolean;
  runId: number;
  step: number;
  memory: TradeTypesBridgeMemory;
  recognized: string[];
...
export const useTradeTypesBridgeStore = create<TradeTypesBridgeState>((set, get) => ({
  active: false,
  runId: 0,
  step: 0,
  memory: loadMemory(),
  ...
  markBridgeCompleted: () => {
    const memory = {
      ...
      conceptsMastered: Array.from(
        new Set([...get().memory.conceptsMastered, "trade-types-certified"]),
      ).slice(0, 50),
    };
    saveMemory(memory);
    set({ memory });
  ...
  markRecognized: (id) => {
    ...
    const checks = ["identify-market", "identify-limit", "identify-stop"];
    if (checks.every((c) => recognized.includes(c)) && !recognized.includes("trade-types-pretrade-ready")) {
      recognized = [...recognized, "trade-types-pretrade-ready"];
    }
    const mastered = new Set(s.memory.conceptsMastered);
    mastered.add(id);
    if (recognized.includes("trade-types-pretrade-ready")) mastered.add("trade-types-pretrade-ready");
