...
import { MarketStatePlayground } from "@/components/terminal/explain/market-state/MarketStatePlayground";
import { MARKET_STATE_SCENES } from "@/lib/education/marketStateScenes";
...
const scenes = MARKET_STATE_SCENES;

export function MarketStateSimulator() {
  const active = useMarketStateLessonStore((s) => s.active);
  const runId = useMarketStateLessonStore((s) => s.runId);
  const startStep = useMarketStateLessonStore((s) => s.startStep);
  const close = useMarketStateLessonStore((s) => s.close);
  const restart = useMarketStateLessonStore((s) => s.restart);
  const markStep = useMarketStateLessonStore((s) => s.markStep);
  const markCompleted = useMarketStateLessonStore((s) => s.markCompleted);
  const startBridge = useMarketStateBridgeStore((s) => s.start);
  const markSimulatorCompleted = useMarketStateBridgeStore((s) => s.markSimulatorCompleted);
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col bg-slate-950" role="dialog" aria-modal="true" aria-label="Market State Layer simulator">
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className={cn(TERMINAL_TYPO.label, "truncate text-violet-200")}>MARKET STATE LAYER · SIMULATOR</span>
          <span className={cn(TERMINAL_TYPO.micro, "shrink-0 text-violet-600")}>PLATFORM</span>
        </div>
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-4">
        <div className="grid w-full max-w-3xl grid-cols-1 items-center gap-6 md:grid-cols-2">
          <MarketStatePlayground visual={scene.visual} reduceMotion={reduceMotion} sceneKey={sceneKey} animate={playing} />
