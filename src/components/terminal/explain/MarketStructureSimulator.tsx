...
import { MarketStructurePlayground } from "@/components/terminal/explain/market-structure/MarketStructurePlayground";
import { MARKET_STRUCTURE_SCENES } from "@/lib/education/marketStructureScenes";
...
const scenes = MARKET_STRUCTURE_SCENES;

export function MarketStructureSimulator() {
  const active = useMarketStructureLessonStore((s) => s.active);
  const runId = useMarketStructureLessonStore((s) => s.runId);
  const startStep = useMarketStructureLessonStore((s) => s.startStep);
  const close = useMarketStructureLessonStore((s) => s.close);
  const restart = useMarketStructureLessonStore((s) => s.restart);
  const markStep = useMarketStructureLessonStore((s) => s.markStep);
  const markCompleted = useMarketStructureLessonStore((s) => s.markCompleted);
  const startBridge = useMarketStructureBridgeStore((s) => s.start);
  const markSimulatorCompleted = useMarketStructureBridgeStore((s) => s.markSimulatorCompleted);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Market structure simulator"
    >
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className={cn(TERMINAL_TYPO.label, "text-violet-200")}>MARKET STRUCTURE · SIMULATOR</span>
        </div>
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-4">
        <div className="grid w-full max-w-3xl grid-cols-1 items-center gap-6 md:grid-cols-2">
          <MarketStructurePlayground visual={scene.visual} sceneKey={sceneKey} animate={playing} />
