...
import { LiquidationsPlayground } from "@/components/terminal/explain/liquidations/LiquidationsPlayground";
import { LIQUIDATIONS_SCENES } from "@/lib/education/liquidationsScenes";
...
const scenes = LIQUIDATIONS_SCENES;

export function LiquidationsSimulator() {
  const active = useLiquidationsStore((s) => s.active);
  const runId = useLiquidationsStore((s) => s.runId);
  const startStep = useLiquidationsStore((s) => s.startStep);
  const close = useLiquidationsStore((s) => s.close);
  const restart = useLiquidationsStore((s) => s.restart);
  const markStep = useLiquidationsStore((s) => s.markStep);
  const markCompleted = useLiquidationsStore((s) => s.markCompleted);
  const startBridge = useLiquidationsBridgeStore((s) => s.start);
  const markSimulatorCompleted = useLiquidationsBridgeStore((s) => s.markSimulatorCompleted);
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col bg-slate-950 transition-opacity duration-200" role="dialog" aria-modal="true" aria-label="Liquidations simulator">
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          <GraduationCap className="h-3.5 w-3.5 shrink-0 text-rose-400" />
          <span className={cn(TERMINAL_TYPO.label, "text-rose-200")}>LIQUIDATIONS · SIMULATOR</span>
        </div>
      ...
      <div className="flex shrink-0 gap-0.5 px-3 py-1.5" aria-hidden="true">
        {scenes.map((s, i) => (
          <span key={s.id} className={cn("h-1 flex-1 transition-colors duration-300", i < index ? "bg-rose-800" : i === index ? "bg-rose-400" : "bg-slate-800")} />
        ))}
      </div>
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-4">
        <div className="grid w-full max-w-3xl grid-cols-1 items-center gap-6 md:grid-cols-2">
          <LiquidationsPlayground visual={scene.visual} reduceMotion={reduceMotion} sceneKey={sceneKey} animate={playing} />
          <div className="border border-slate-700/60 bg-slate-900/40 p-4">
            <span className={cn(TERMINAL_TYPO.micro, "text-rose-500")}>{scene.chapter}</span>
      ...
      <div className="flex shrink-0 flex-wrap items-center justify-center gap-2 border-t border-slate-800 px-3 py-2.5">
        ...
        <button type="button" onClick={togglePlay} className={cn(TERMINAL_TYPO.micro, "border px-2 py-1", playing ? "border-rose-500/60 text-rose-200" : "border-slate-700 text-slate-300")}>
        ...
        {isLast ? (
          <button type="button" onClick={findItLive} className={cn(TERMINAL_TYPO.micro, "flex items-center gap-1 border border-rose-500/70 bg-rose-950/40 px-2 py-1 text-rose-200")}>
            FIND IT LIVE <Radio className="h-3 w-3" />
          </button>
        ) : (
          <button type="button" onClick={() => setIndex(index + 1)} className={cn(TERMINAL_TYPO.micro, "border border-rose-700/50 px-2 py-1 text-rose-300")}>
            <AcademyNextLabel />
          </button>
        )}
      </div>
