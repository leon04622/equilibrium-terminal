...
import { Crosshair, Eye, Radio, Shield } from "lucide-react";
import { cn } from "@/lib/utils";
import { TERMINAL_TYPO } from "@/lib/theme";
import { primeLessonVoice } from "@/lib/education/LessonNarrator";
import { LiveRiskManagementCoach, type CoachState } from "@/lib/education/liveRiskManagementCoach";
  ...
  return (
    <div className="flex items-start gap-1.5 py-1">
      ...
      <div className="min-w-0">
        <span className={cn(TERMINAL_TYPO.micro, "text-emerald-500")}>{label}</span>
...
export function LiveRiskManagementCoachPanel({ panelId }: { panelId: string }) {
  ...
  const simulatorActive = useRiskManagementStore((s) => s.active);
  const bridgeActive = useRiskManagementBridgeStore((s) => s.active);

  if (panelId !== "ticket") return null;
  if (simulatorActive || bridgeActive) return null;

  const card = LiveRiskManagementCoach.operatorCoach(snapshot);
    ...
    <div className={cn("mb-2 border bg-slate-900/40 p-2", STATE_BORDER[card.state])}>
      <div className="mb-1 flex items-center justify-between">
        <span className="flex items-center gap-1.5">
          ...
          <span className={cn(TERMINAL_TYPO.label, "text-slate-100")}>RISK COACH</span>
        </span>
      ...
      <Field icon={<Shield className="h-3 w-3" />} label="WHAT TO WATCH">
        {card.whatToWatch}
      </Field>
      <div className="mt-1.5 border border-emerald-800/40 bg-emerald-950/20 px-2 py-1">
        <p className={cn(TERMINAL_TYPO.micro, "text-emerald-400")}>NOW</p>
        <p className="font-mono text-[10px] leading-snug text-emerald-100">{card.alertLine}</p>
      </div>
      <div className="mt-1.5 flex gap-1">
        ...
        <button
          ...
          className={cn(TERMINAL_TYPO.micro, "flex-1 border border-emerald-700/50 bg-emerald-950/30 py-1 text-emerald-300")}
        >
          WALK ME THROUGH IT
        </button>
      </div>
