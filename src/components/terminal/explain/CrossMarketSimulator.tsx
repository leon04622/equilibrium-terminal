...
import { CrossMarketPlayground } from "@/components/terminal/explain/cross-market/CrossMarketPlayground";
import { CROSS_MARKET_SCENES } from "@/lib/education/crossMarketScenes";
...
const scenes = CROSS_MARKET_SCENES;

export function CrossMarketSimulator() {
  const active = useCrossMarketLessonStore((s) => s.active);
  const runId = useCrossMarketLessonStore((s) => s.runId);
  const startStep = useCrossMarketLessonStore((s) => s.startStep);
  const close = useCrossMarketLessonStore((s) => s.close);
  const restart = useCrossMarketLessonStore((s) => s.restart);
  const markStep = useCrossMarketLessonStore((s) => s.markStep);
  const markCompleted = useCrossMarketLessonStore((s) => s.markCompleted);
  const startBridge = useCrossMarketBridgeStore((s) => s.start);
  const markSimulatorCompleted = useCrossMarketBridgeStore((s) => s.markSimulatorCompleted);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Cross-market analysis simulator"
    >
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className={cn(TERMINAL_TYPO.label, "text-cyan-200")}>CROSS-MARKET · SIMULATOR</span>
        </div>
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-4">
        <div className="grid w-full max-w-3xl grid-cols-1 items-center gap-6 md:grid-cols-2">
          <CrossMarketPlayground visual={scene.visual} sceneKey={sceneKey} animate={playing} />
