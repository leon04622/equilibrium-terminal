...
import { MarketMemoryPlayground } from "@/components/terminal/explain/market-memory/MarketMemoryPlayground";
import { MARKET_MEMORY_SCENES } from "@/lib/education/marketMemoryScenes";
...
const scenes = MARKET_MEMORY_SCENES;

export function MarketMemorySimulator() {
  const active = useMarketMemoryLessonStore((s) => s.active);
  const runId = useMarketMemoryLessonStore((s) => s.runId);
  const startStep = useMarketMemoryLessonStore((s) => s.startStep);
  const close = useMarketMemoryLessonStore((s) => s.close);
  const restart = useMarketMemoryLessonStore((s) => s.restart);
  const markStep = useMarketMemoryLessonStore((s) => s.markStep);
  const markCompleted = useMarketMemoryLessonStore((s) => s.markCompleted);
  const startBridge = useMarketMemoryBridgeStore((s) => s.start);
  const markSimulatorCompleted = useMarketMemoryBridgeStore((s) => s.markSimulatorCompleted);
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col bg-slate-950" role="dialog" aria-modal="true" aria-label="Market Memory Archive simulator">
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className={cn(TERMINAL_TYPO.label, "truncate text-cyan-200")}>MARKET MEMORY ARCHIVE · SIMULATOR</span>
          <span className={cn(TERMINAL_TYPO.micro, "shrink-0 text-cyan-600")}>PLATFORM</span>
        </div>
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-4">
        <div className="grid w-full max-w-3xl grid-cols-1 items-center gap-6 md:grid-cols-2">
          <MarketMemoryPlayground visual={scene.visual} reduceMotion={reduceMotion} sceneKey={sceneKey} animate={playing} />
      ...
      <div className="flex shrink-0 flex-wrap items-center justify-center gap-2 border-t border-slate-800 px-3 py-2.5">
        ...
        {isLast ? (
          ...
          <button type="button" aria-label="Next step" onClick={() => setIndex(index + 1)} className={cn(TERMINAL_TYPO.micro, "border border-cyan-700/50 px-2 py-1 text-cyan-300")}>
            <AcademyNextLabel />
          </button>
        )}
      </div>
