...
import {
  MARKET_STATE_BRIDGE_PANEL,
  MARKET_STATE_BRIDGE_STEPS,
  MARKET_STATE_REQUIRED_CONCEPTS,
  type MSBridgeRegion,
} from "@/lib/education/marketStateBridgeSteps";
...
const steps = MARKET_STATE_BRIDGE_STEPS;

function msContext() {
  return MarketStateCoach.contextLive();
}

function panelEl(): HTMLElement | null {
  if (typeof document === "undefined") return null;
  return document.querySelector<HTMLElement>(`[data-marketstate-panel="${MARKET_STATE_BRIDGE_PANEL}"]`);
}

function regionEl(region: MSBridgeRegion): HTMLElement | null {
  const panel = panelEl();
  if (!panel) return null;
  if (!region || region === "panel") return panel;
  return panel.querySelector<HTMLElement>(`[data-marketstate-region="${region}"]`) ?? panel;
}

function spotlightRegion(step: (typeof steps)[number] | undefined): MSBridgeRegion {
  if (!step) return null;
...
export function MarketStateLiveBridge() {
  const active = useMarketStateBridgeStore((s) => s.active);
  const runId = useMarketStateBridgeStore((s) => s.runId);
  const close = useMarketStateBridgeStore((s) => s.close);
  const markBridgeCompleted = useMarketStateBridgeStore((s) => s.markBridgeCompleted);
  const markRecognized = useMarketStateBridgeStore((s) => s.markRecognized);
  const markDecision = useMarketStateBridgeStore((s) => s.markDecision);
  const recognized = useMarketStateBridgeStore((s) => s.recognized);
  const setStoreStep = useMarketStateBridgeStore((s) => s.setStep);
  ...
  const rect = useAcademyBridgeSpotlight({
    ...
    step,
    spotlightRegion,
    regionEl: (region) => regionEl(region as MSBridgeRegion),
    panelEl,
  });
  ...
  const enter = useCallback(
    (i: number, force = false) => {
      ...
      const s = steps[i];
      if (!s) return;
      setHighlightPanel(MARKET_STATE_BRIDGE_PANEL);
      terminalBus.emit("widget:focus", { widgetId: MARKET_STATE_BRIDGE_PANEL });
      if (i >= steps.length - 1) markBridgeCompleted();
      const snap = msContext();
      const coachText = s.coach(snap);
      const extras =
        s.id === "workflow"
          ? MarketStateCoach.workflowSteps().map((w) => `${w.order}. ${w.label}: ${w.note}`)
          : [];
  ...
  useEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      if (!target) return;
      const panel = target.closest(`[data-marketstate-panel="${MARKET_STATE_BRIDGE_PANEL}"]`);
      const regionNode = target.closest("[data-marketstate-region]") as HTMLElement | null;
      if (!panel || !regionNode) return;
      const region = regionNode.getAttribute("data-marketstate-region") as Exclude<MSBridgeRegion, null> | null;
  ...
  const workflow = MarketStateCoach.workflowSteps();
      ...
      {rect ? (
        <div
          className={cn(
            ...
            step.mode === "recognize"
              ? "animate-pulse ring-violet-300 shadow-[0_0_40px_rgba(167,139,250,0.55)]"
              : "ring-violet-400/80 shadow-[0_0_30px_rgba(167,139,250,0.35)]",
          )}
      ...
      <div data-academy-bridge-chrome className="fixed inset-x-0 top-12 z-[160] flex justify-center px-3 pt-2">
        <div className={cn("w-full max-w-xl border bg-slate-950/95 backdrop-blur", isInteractive ? "border-violet-600/60" : "border-violet-700/50")}>
          <div className="flex items-center justify-between border-b border-slate-800 px-3 py-1.5">
            <span className={cn(TERMINAL_TYPO.label, "text-violet-200")}>MARKET STATE BRIDGE · {step.chapter}</span>
          ...
          <div className="px-3 py-2.5">
            ...
            {step.mode === "recognize" ? (
              <p className={cn("mt-2 flex items-center gap-1.5 font-mono text-xs", feedback === "correct" ? "text-violet-300" : feedback === "wrong" ? "text-rose-300" : "text-violet-200")}>
                {feedback === "correct" ? (
                  <>
            ...
            {step.id === "workflow" ? (
              <div className="mt-2 rounded-md border border-violet-800/40 bg-violet-950/15 p-2">
                <p className={cn(TERMINAL_TYPO.label, "mb-1 text-violet-200")}>MARKET STATE WORKFLOW</p>
                {workflow.map((w) => (
              ...
              <div className="mt-2 space-y-2">
                <div className="flex items-center gap-2 border border-violet-700/50 bg-violet-950/20 px-3 py-2">
                  ...
                  <span className="font-mono text-sm font-bold uppercase tracking-wide text-violet-200">
                    Market State Layer Certified
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-1 sm:grid-cols-3">
                  {MARKET_STATE_REQUIRED_CONCEPTS.map((c) => (
                    <div
                      key={c}
                      className={cn(
                        "flex items-center gap-1 rounded-sm border px-2 py-1 font-mono text-[10px] uppercase",
                        recognized.includes(c) || c === "market-state-certified"
          ...
          <div className="flex flex-wrap justify-center gap-2 border-t border-slate-800 px-3 py-2">
            ...
            {isInteractive && !interactiveDone ? (
              ...
              <button type="button" aria-label="Next step" onClick={() => setIndex((i) => Math.min(i + 1, steps.length - 1))} className={cn(TERMINAL_TYPO.micro, "border border-violet-700/50 px-2 py-1 text-violet-300")}>
                <AcademyNextLabel />
              </button>
            )}
          </div>
