...
import { LiveBookCoach, type CoachState } from "@/lib/education/liveBookCoach";
import { terminalBus } from "@/store/eventBus";
import { useHyperliquidStore } from "@/store/hyperliquidStore";
import { useLessonBridgeStore } from "@/store/useLessonBridgeStore";

/**
 * PHASE 1 — the Operator Coach as a senior trader pointing at the screen.
 * Four short, operational fields read from the LIVE book. Only renders for the
 * order book panel; isolated as its own component so it re-renders on book
 * ticks without re-rendering the whole side panel.
 */
...
export function LiveOperatorCoach({ panelId }: { panelId: string }) {
  // bookVersion guarantees a re-render on each book tick.
  ...
  if (panelId !== "hyperbook") return null;

  const card = LiveBookCoach.operatorCoach(book);
    ...
    <div className={cn("mb-2 border bg-slate-900/40 p-2", STATE_BORDER[card.state])}>
      <div className="mb-1 flex items-center justify-between">
        <span className="flex items-center gap-1.5">
          ...
          <span className={cn(TERMINAL_TYPO.label, "text-slate-100")}>OPERATOR COACH</span>
        </span>
      ...
      <div className="mt-1.5 flex gap-1">
        <button
          type="button"
          onClick={() => terminalBus.emit("widget:focus", { widgetId: "hyperbook" })}
          className={cn(
            TERMINAL_TYPO.micro,
            "flex-1 border border-slate-700 py-1 text-slate-300 hover:border-slate-500",
          )}
        >
          HIGHLIGHT BOOK
        </button>
        <button
          ...
          className={cn(
            TERMINAL_TYPO.micro,
            "flex-1 border border-cyan-700/50 bg-cyan-950/30 py-1 text-cyan-300 hover:bg-cyan-950/50",
          )}
        >
          WALK ME THROUGH IT
        </button>
