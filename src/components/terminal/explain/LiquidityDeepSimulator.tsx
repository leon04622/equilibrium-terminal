...
import { LiquidityDeepPlayground } from "@/components/terminal/explain/liquidity-deep/LiquidityDeepPlayground";
import { LIQUIDITY_DEEP_SCENES } from "@/lib/education/liquidityDeepScenes";
...
const scenes = LIQUIDITY_DEEP_SCENES;

export function LiquidityDeepSimulator() {
  const active = useLiquidityDeepLessonStore((s) => s.active);
  const runId = useLiquidityDeepLessonStore((s) => s.runId);
  const startStep = useLiquidityDeepLessonStore((s) => s.startStep);
  const close = useLiquidityDeepLessonStore((s) => s.close);
  const restart = useLiquidityDeepLessonStore((s) => s.restart);
  const markStep = useLiquidityDeepLessonStore((s) => s.markStep);
  const markCompleted = useLiquidityDeepLessonStore((s) => s.markCompleted);
  const startBridge = useLiquidityDeepBridgeStore((s) => s.start);
  const markSimulatorCompleted = useLiquidityDeepBridgeStore((s) => s.markSimulatorCompleted);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Liquidity depth simulator"
    >
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className={cn(TERMINAL_TYPO.label, "text-cyan-200")}>LIQUIDITY · SIMULATOR</span>
        </div>
        ...
        {scenes.map((s, i) => (
          <span
            ...
            className={cn(
              ...
              i < index ? "bg-cyan-800" : i === index ? "bg-cyan-400" : "bg-slate-800",
            )}
          />
        ))}
      </div>
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-4">
        <div className="grid w-full max-w-3xl grid-cols-1 items-center gap-6 md:grid-cols-2">
          <LiquidityDeepPlayground visual={scene.visual} sceneKey={sceneKey} animate={playing} />
      ...
      <div className="flex shrink-0 flex-wrap items-center justify-center gap-2 border-t border-slate-800 px-3 py-2.5">
        ...
        <button
          type="button"
          onClick={togglePlay}
          className={cn(TERMINAL_TYPO.micro, "border px-2 py-1", playing ? "border-cyan-500/60 text-cyan-200" : "border-slate-700 text-slate-300")}
        >
        ...
        {isLast ? (
          <button
            type="button"
            onClick={findItLive}
            className={cn(TERMINAL_TYPO.micro, "flex items-center gap-1 border border-cyan-500/70 bg-cyan-950/40 px-2 py-1 text-cyan-200")}
          >
