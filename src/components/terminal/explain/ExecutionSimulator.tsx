...
import { ExecutionPlayground } from "@/components/terminal/explain/execution/ExecutionPlayground";
import { EXECUTION_SCENES } from "@/lib/education/executionScenes";
...
const scenes = EXECUTION_SCENES;

export function ExecutionSimulator() {
  const active = useExecutionLessonStore((s) => s.active);
  const runId = useExecutionLessonStore((s) => s.runId);
  const startStep = useExecutionLessonStore((s) => s.startStep);
  const close = useExecutionLessonStore((s) => s.close);
  const restart = useExecutionLessonStore((s) => s.restart);
  const markStep = useExecutionLessonStore((s) => s.markStep);
  const markCompleted = useExecutionLessonStore((s) => s.markCompleted);
  const startBridge = useExecutionLessonBridgeStore((s) => s.start);
  const markSimulatorCompleted = useExecutionLessonBridgeStore((s) => s.markSimulatorCompleted);
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col bg-slate-950 transition-opacity duration-200" role="dialog" aria-modal="true" aria-label="Execution simulator">
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className={cn(TERMINAL_TYPO.label, "text-violet-200")}>EXECUTION · SIMULATOR</span>
        </div>
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-4">
        <div className="grid w-full max-w-3xl grid-cols-1 items-center gap-6 md:grid-cols-2">
          <ExecutionPlayground visual={scene.visual} reduceMotion={reduceMotion} sceneKey={sceneKey} animate={playing} />
