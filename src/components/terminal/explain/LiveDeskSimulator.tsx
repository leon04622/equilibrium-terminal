...
const scenes = LIVE_DESK_SCENES;

export function LiveDeskSimulator() {
  const active = useLiveDeskLessonStore((s) => s.active);
  const runId = useLiveDeskLessonStore((s) => s.runId);
  const startStep = useLiveDeskLessonStore((s) => s.startStep);
  const close = useLiveDeskLessonStore((s) => s.close);
  const restart = useLiveDeskLessonStore((s) => s.restart);
  const markStep = useLiveDeskLessonStore((s) => s.markStep);
  const markCompleted = useLiveDeskLessonStore((s) => s.markCompleted);
  const startBridge = useLiveDeskBridgeStore((s) => s.start);
  const markSimulatorCompleted = useLiveDeskBridgeStore((s) => s.markSimulatorCompleted);
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col bg-slate-950" role="dialog" aria-modal="true" aria-label="Live Desk simulator">
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className={cn(TERMINAL_TYPO.label, "truncate text-cyan-200")}>LIVE DESK · SIMULATOR</span>
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-4">
        <div className="grid w-full max-w-3xl grid-cols-1 items-center gap-6 md:grid-cols-2">
          <LiveDeskPlayground visual={scene.visual} reduceMotion={reduceMotion} sceneKey={sceneKey} animate={playing} />
