...
import { PortfolioRiskPlayground } from "@/components/terminal/explain/portfolio-risk/PortfolioRiskPlayground";
import { PORTFOLIO_RISK_SCENES } from "@/lib/education/portfolioRiskScenes";
...
const scenes = PORTFOLIO_RISK_SCENES;

export function PortfolioRiskSimulator() {
  const active = usePortfolioRiskStore((s) => s.active);
  const runId = usePortfolioRiskStore((s) => s.runId);
  const startStep = usePortfolioRiskStore((s) => s.startStep);
  const close = usePortfolioRiskStore((s) => s.close);
  const restart = usePortfolioRiskStore((s) => s.restart);
  const markStep = usePortfolioRiskStore((s) => s.markStep);
  const markCompleted = usePortfolioRiskStore((s) => s.markCompleted);
  const startBridge = usePortfolioRiskBridgeStore((s) => s.start);
  const markSimulatorCompleted = usePortfolioRiskBridgeStore((s) => s.markSimulatorCompleted);
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col bg-slate-950 transition-opacity duration-200" role="dialog" aria-modal="true" aria-label="Portfolio risk simulator">
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className={cn(TERMINAL_TYPO.label, "text-amber-200")}>PORTFOLIO RISK · SIMULATOR</span>
        </div>
      ...
      <div className="flex shrink-0 gap-0.5 px-3 py-1.5" aria-hidden="true">
        {scenes.map((s, i) => (
          <span key={s.id} className={cn("h-1 flex-1 transition-colors duration-300", i < index ? "bg-amber-800" : i === index ? "bg-amber-400" : "bg-slate-800")} />
        ))}
      </div>
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-4">
        <div className="grid w-full max-w-3xl grid-cols-1 items-center gap-6 md:grid-cols-2">
          <PortfolioRiskPlayground visual={scene.visual} reduceMotion={reduceMotion} sceneKey={sceneKey} animate={playing} />
