...
import { FirstTradePlayground } from "@/components/terminal/explain/first-trade/FirstTradePlayground";
import { FIRST_TRADE_SCENES } from "@/lib/education/firstTradeScenes";
...
const scenes = FIRST_TRADE_SCENES;

export function FirstTradeSimulator() {
  const active = useFirstTradeLessonStore((s) => s.active);
  const runId = useFirstTradeLessonStore((s) => s.runId);
  const startStep = useFirstTradeLessonStore((s) => s.startStep);
  const close = useFirstTradeLessonStore((s) => s.close);
  const restart = useFirstTradeLessonStore((s) => s.restart);
  const markStep = useFirstTradeLessonStore((s) => s.markStep);
  const markCompleted = useFirstTradeLessonStore((s) => s.markCompleted);
  const startBridge = useFirstTradeBridgeStore((s) => s.start);
  const markSimulatorCompleted = useFirstTradeBridgeStore((s) => s.markSimulatorCompleted);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="First trade checklist simulator"
    >
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className={cn(TERMINAL_TYPO.label, "text-cyan-200")}>FIRST TRADE · SIMULATOR</span>
        </div>
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-4">
        <div className="grid w-full max-w-3xl grid-cols-1 items-center gap-6 md:grid-cols-2">
          <FirstTradePlayground visual={scene.visual} sceneKey={sceneKey} animate={playing} />
