...
import {
  RISK_MANAGEMENT_BRIDGE_PANEL,
  RISK_MANAGEMENT_BRIDGE_STEPS,
  RISK_MANAGEMENT_REQUIRED_CONCEPTS,
  type RiskBridgePanel,
  type RiskBridgeRegion,
} from "@/lib/education/riskManagementBridgeSteps";
...
const steps = RISK_MANAGEMENT_BRIDGE_STEPS;

function estimateMs(text: string): number {
  return Math.max(2800, text.length * 58);
}

const PANEL_ATTR: Record<RiskBridgePanel, string> = {
  ticket: "data-trade-panel",
  portfoliodesk: "data-portfolio-panel",
};

const REGION_ATTR: Record<RiskBridgePanel, string> = {
  ticket: "data-trade-region",
  portfoliodesk: "data-portfolio-region",
};

const PANEL_ID: Record<RiskBridgePanel, string> = {
  ticket: "ticket",
  portfoliodesk: RISK_MANAGEMENT_BRIDGE_PANEL,
};

function panelEl(bridgePanel: RiskBridgePanel): HTMLElement | null {
...
function regionEl(bridgePanel: RiskBridgePanel, region: RiskBridgeRegion): HTMLElement | null {
...
export function RiskManagementLiveBridge() {
  const active = useRiskManagementBridgeStore((s) => s.active);
  const runId = useRiskManagementBridgeStore((s) => s.runId);
  const close = useRiskManagementBridgeStore((s) => s.close);
  const markBridgeCompleted = useRiskManagementBridgeStore((s) => s.markBridgeCompleted);
  const markRecognized = useRiskManagementBridgeStore((s) => s.markRecognized);
  const markDecision = useRiskManagementBridgeStore((s) => s.markDecision);
  const recognized = useRiskManagementBridgeStore((s) => s.recognized);
  const setStoreStep = useRiskManagementBridgeStore((s) => s.setStep);
  ...
  const rect = useAcademyBridgeSpotlight({
    ...
    step,
    getTargetEl: (s) =>
      s ? regionEl(s.bridgePanel, bridgeRecognizeRegion(s) as RiskBridgeRegion) : null,
  });
  ...
  const enter = useCallback(
    (i: number) => {
      ...
      const extras =
        s.id === "pre-risk"
          ? LiveRiskManagementCoach.preRiskChecklist().map((item) => `${item.label}: ${item.note}`)
          : [];
      ...
      speakAcademyBridgeStep(s, coachText, snap, {
        ...
        scrollTarget:
          s.mode === "recognize"
            ? regionEl(s.bridgePanel, bridgeRecognizeRegion(s) as RiskBridgeRegion)
            : undefined,
        scrollSmooth: true,
  ...
  useEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      const region = regionNode.getAttribute(regionAttr) as Exclude<RiskBridgeRegion, null> | null;
  ...
  const preRiskItems = LiveRiskManagementCoach.preRiskChecklist();

  return (
    <>
      {rect ? (
        <div
          className="pointer-events-none fixed z-[150] rounded-md ring-2 ring-emerald-400/80 shadow-[0_0_30px_rgba(52,211,153,0.35)] transition-all duration-300"
      ...
      <div data-academy-bridge-chrome className="fixed inset-x-0 top-12 z-[160] flex justify-center px-3 pt-2">
        <div
          className={cn(
            ...
            isInteractive ? "border-emerald-600/60" : "border-emerald-700/50",
          )}
        >
          <div className="flex items-center justify-between border-b border-slate-800 px-3 py-1.5">
            <span className={cn(TERMINAL_TYPO.label, "text-emerald-200")}>RISK MANAGEMENT BRIDGE · {step.chapter}</span>
          ...
          <div className="px-3 py-2.5">
            ...
            {whyCare ? (
              <p className="mt-1 font-mono text-[10px] text-emerald-300/90">
                <span className="text-emerald-500">WHY CARE </span>
                {whyCare}
              </p>
            ) : null}
            ...
            {step.mode === "recognize" ? (
              <p
                className={cn(
                  ...
                  feedback === "correct" ? "text-emerald-300" : feedback === "wrong" ? "text-amber-300" : "text-emerald-200",
                )}
              >
                {feedback === "correct" ? (
                  <>
            ...
            {step.id === "pre-risk" ? (
              <div className="mt-2 rounded-md border border-emerald-800/40 bg-emerald-950/15 p-2">
                <p className={cn(TERMINAL_TYPO.label, "mb-1 text-emerald-200")}>PRE-TRADE RISK CHECK</p>
            ...
            {step.id === "certified" ? (
              <div className="mt-2 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-1">
                  {RISK_MANAGEMENT_REQUIRED_CONCEPTS.map((c) => (
                    <div
                      key={c}
                      className={cn(
                        "flex items-center gap-1 rounded-sm border px-2 py-1 font-mono text-[10px] uppercase",
                        recognized.includes(c) || c === "risk-management-certified"
          ...
          <div className="flex flex-wrap justify-center gap-2 border-t border-slate-800 px-3 py-2">
            ...
            {!isInteractive ? (
              <button
                ...
                className={cn(TERMINAL_TYPO.micro, "border px-2 py-1", playing ? "border-emerald-500/60 text-emerald-200" : "text-slate-300")}
              >
