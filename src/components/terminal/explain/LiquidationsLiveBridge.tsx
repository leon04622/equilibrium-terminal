...
export function LiquidationsLiveBridge() {
  ...
  const rect = useAcademyBridgeSpotlight({
    ...
    step,
    getTargetEl: (s) =>
      s ? regionEl(s.bridgePanel, bridgeRecognizeRegion(s) as LiqBridgeRegion) : null,
  });
  ...
  const enter = useCallback(
    (i: number) => {
      ...
      speakAcademyBridgeStep(s, coachText, snap, {
        ...
        scrollTarget:
          s.mode === "recognize"
            ? regionEl(s.bridgePanel, bridgeRecognizeRegion(s) as LiqBridgeRegion)
            : undefined,
        scrollSmooth: true,
  ...
  }, [active, runId, setHighlightPanel, setDerivTab]);
