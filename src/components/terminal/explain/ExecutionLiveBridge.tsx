...
import { LiveExecutionCoach } from "@/lib/education/liveExecutionCoach";
import {
  EXECUTION_BRIDGE_PANEL,
  EXECUTION_BRIDGE_STEPS,
  EXECUTION_REQUIRED_CONCEPTS,
  type ExecBridgePanel,
  type ExecBridgeRegion,
} from "@/lib/education/executionBridgeSteps";
...
const steps = EXECUTION_BRIDGE_STEPS;

function estimateMs(text: string): number {
  return Math.max(2800, text.length * 58);
}

const PANEL_ATTR: Record<ExecBridgePanel, string> = {
  hyperbook: "data-book-panel",
  ticket: "data-trade-panel",
  slippageradar: "data-slip-panel",
  execintel: "data-execintel-panel",
};

const REGION_ATTR: Record<ExecBridgePanel, string> = {
  hyperbook: "data-book-region",
  ticket: "data-trade-region",
  slippageradar: "data-slip-region",
  execintel: "data-execintel-region",
};

const PANEL_ID: Record<ExecBridgePanel, string> = {
  hyperbook: "hyperbook",
  ticket: EXECUTION_BRIDGE_PANEL,
...
function execContext() {
  ...
  const slippage = useExecutionIntelligenceStore.getState().slippage;
  const stress = useMarketAtmosphereStore.getState().stress;
  const regime = useMarketAtmosphereStore.getState().regime.regime;
  const execConfidence = useExecutionIntelligenceStore.getState().executionConfidence;
  return LiveExecutionCoach.context(book, slippage, regime, stress.score, stress.velocityRatio, execConfidence);
}

function panelEl(bridgePanel: ExecBridgePanel): HTMLElement | null {
...
function regionEl(bridgePanel: ExecBridgePanel, region: ExecBridgeRegion): HTMLElement | null {
...
export function ExecutionLiveBridge() {
  const active = useExecutionLessonBridgeStore((s) => s.active);
  const runId = useExecutionLessonBridgeStore((s) => s.runId);
  const close = useExecutionLessonBridgeStore((s) => s.close);
  const markBridgeCompleted = useExecutionLessonBridgeStore((s) => s.markBridgeCompleted);
  const markRecognized = useExecutionLessonBridgeStore((s) => s.markRecognized);
  const markDecision = useExecutionLessonBridgeStore((s) => s.markDecision);
  const recognized = useExecutionLessonBridgeStore((s) => s.recognized);
  const setStoreStep = useExecutionLessonBridgeStore((s) => s.setStep);

  const setHighlightPanel = useOperatorGuideStore((s) => s.setHighlightPanel);
  const setExecIntelTab = useExecutionAnalyticsStore((s) => s.setActiveTab);
  ...
  const rect = useAcademyBridgeSpotlight({
    ...
    step,
    getTargetEl: (s) =>
      s ? regionEl(s.bridgePanel, bridgeRecognizeRegion(s) as ExecBridgeRegion) : null,
  });
  ...
  const enter = useCallback(
    (i: number) => {
      ...
      const s = steps[i];
      if (!s) return;
      if (s.execIntelTab) setExecIntelTab(s.execIntelTab);
      ...
      const extras =
        s.id === "pre-exec"
          ? LiveExecutionCoach.preExecChecklist().map((item) => `${item.label}: ${item.note}`)
          : [];
      ...
      speakAcademyBridgeStep(s, coachText, ctx, {
        ...
        scrollTarget:
          s.mode === "recognize"
            ? regionEl(s.bridgePanel, bridgeRecognizeRegion(s) as ExecBridgeRegion)
            : undefined,
        scrollSmooth: true,
    ...
    [supported, clearTimers, markBridgeCompleted, setHighlightPanel, setExecIntelTab],
  );
  ...
  useEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      const region = regionNode.getAttribute(regionAttr) as Exclude<ExecBridgeRegion, null> | null;
  ...
  const preExecItems = LiveExecutionCoach.preExecChecklist();

  return (
    <>
      {rect ? (
        <div
          className="pointer-events-none fixed z-[150] rounded-md ring-2 ring-violet-400/80 shadow-[0_0_30px_rgba(167,139,250,0.35)] transition-all duration-300"
      ...
      <div data-academy-bridge-chrome className="fixed inset-x-0 top-12 z-[160] flex justify-center px-3 pt-2">
        <div
          className={cn(
            ...
            isInteractive ? "border-violet-600/60" : "border-violet-700/50",
          )}
        >
          <div className="flex items-center justify-between border-b border-slate-800 px-3 py-1.5">
            <span className={cn(TERMINAL_TYPO.label, "text-violet-200")}>EXECUTION BRIDGE · {step.chapter}</span>
          ...
          <div className="px-3 py-2.5">
            ...
            {whyCare ? (
              <p className="mt-1 font-mono text-[10px] text-violet-300/90">
                <span className="text-violet-500">WHY CARE </span>
                {whyCare}
              </p>
            ) : null}
            ...
            {step.mode === "recognize" ? (
              <p
                className={cn(
                  ...
                  feedback === "correct" ? "text-violet-300" : feedback === "wrong" ? "text-amber-300" : "text-violet-200",
                )}
              >
                {feedback === "correct" ? (
                  <>
            ...
            {step.id === "pre-exec" ? (
              <div className="mt-2 rounded-md border border-violet-800/40 bg-violet-950/15 p-2">
                <p className={cn(TERMINAL_TYPO.label, "mb-1 text-violet-200")}>PRE-TRADE EXECUTION CHECK</p>
                {preExecItems.map((item) => (
                  <div
                    key={item.id}
                    className="flex gap-1.5 border-b border-slate-800 py-1 font-mono text-[10px] text-slate-300 last:border-0"
                  >
                    <Check className="mt-px h-3 w-3 shrink-0 text-violet-400" />
                    <span>
            ...
            {step.id === "certified" ? (
              <div className="mt-2 space-y-2">
                <div className="flex items-center gap-2 border border-violet-700/50 bg-violet-950/20 px-3 py-2">
                  <Award className="h-5 w-5 text-violet-400" />
                  <span className="font-mono text-sm font-bold uppercase tracking-wide text-violet-200">
                    Execution Certified
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-1">
                  {EXECUTION_REQUIRED_CONCEPTS.map((c) => (
                    <div
                      key={c}
                      className={cn(
                        "flex items-center gap-1 rounded-sm border px-2 py-1 font-mono text-[10px] uppercase",
                        recognized.includes(c) || c === "execution-certified"
                          ? "border-violet-700/40 text-violet-200"
          ...
          <div className="flex flex-wrap justify-center gap-2 border-t border-slate-800 px-3 py-2">
            ...
            {!isInteractive ? (
              <button
                ...
                className={cn(TERMINAL_TYPO.micro, "border px-2 py-1", playing ? "border-violet-500/60 text-violet-200" : "text-slate-300")}
              >
            ...
            {isInteractive && !interactiveDone ? (
              ...
              <button type="button" onClick={exit} className={cn(TERMINAL_TYPO.micro, "border border-violet-700/50 px-2 py-1 text-violet-300")}>
                DONE
              </button>
            ) : (
              <button
