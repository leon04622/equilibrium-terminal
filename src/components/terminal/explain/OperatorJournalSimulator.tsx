...
import { OperatorJournalPlayground } from "@/components/terminal/explain/operator-journal/OperatorJournalPlayground";
import { OPERATOR_JOURNAL_SCENES } from "@/lib/education/operatorJournalScenes";
...
const scenes = OPERATOR_JOURNAL_SCENES;

export function OperatorJournalSimulator() {
  const active = useOperatorJournalLessonStore((s) => s.active);
  const runId = useOperatorJournalLessonStore((s) => s.runId);
  const startStep = useOperatorJournalLessonStore((s) => s.startStep);
  const close = useOperatorJournalLessonStore((s) => s.close);
  const restart = useOperatorJournalLessonStore((s) => s.restart);
  const markStep = useOperatorJournalLessonStore((s) => s.markStep);
  const markCompleted = useOperatorJournalLessonStore((s) => s.markCompleted);
  const startBridge = useOperatorJournalBridgeStore((s) => s.start);
  const markSimulatorCompleted = useOperatorJournalBridgeStore((s) => s.markSimulatorCompleted);
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col bg-slate-950" role="dialog" aria-modal="true" aria-label="Operator Journal simulator">
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className={cn(TERMINAL_TYPO.label, "truncate text-cyan-200")}>OPERATOR JOURNAL · SIMULATOR</span>
      ...
      <div className="flex shrink-0 gap-0.5 px-3 py-1.5" aria-hidden="true">
        {scenes.map((s, i) => (
          <span key={s.id} className={cn("h-1 flex-1", i < index ? "bg-cyan-800" : i === index ? "bg-cyan-400" : "bg-slate-800")} />
        ))}
      </div>
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-4">
        <div className="grid w-full max-w-3xl grid-cols-1 items-center gap-6 md:grid-cols-2">
          <OperatorJournalPlayground visual={scene.visual} reduceMotion={reduceMotion} sceneKey={sceneKey} animate={playing} />
