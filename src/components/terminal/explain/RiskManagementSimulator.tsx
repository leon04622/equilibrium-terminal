...
import { RiskManagementPlayground } from "@/components/terminal/explain/risk-management/RiskManagementPlayground";
import { RISK_MANAGEMENT_SCENES } from "@/lib/education/riskManagementScenes";
...
const scenes = RISK_MANAGEMENT_SCENES;

export function RiskManagementSimulator() {
  const active = useRiskManagementStore((s) => s.active);
  const runId = useRiskManagementStore((s) => s.runId);
  const startStep = useRiskManagementStore((s) => s.startStep);
  const close = useRiskManagementStore((s) => s.close);
  const restart = useRiskManagementStore((s) => s.restart);
  const markStep = useRiskManagementStore((s) => s.markStep);
  const markCompleted = useRiskManagementStore((s) => s.markCompleted);
  const startBridge = useRiskManagementBridgeStore((s) => s.start);
  const markSimulatorCompleted = useRiskManagementBridgeStore((s) => s.markSimulatorCompleted);
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col bg-slate-950 transition-opacity duration-200" role="dialog" aria-modal="true" aria-label="Risk management simulator">
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className={cn(TERMINAL_TYPO.label, "text-emerald-200")}>RISK MANAGEMENT · SIMULATOR</span>
        </div>
      ...
      <div className="flex shrink-0 gap-0.5 px-3 py-1.5" aria-hidden="true">
        {scenes.map((s, i) => (
          <span key={s.id} className={cn("h-1 flex-1 transition-colors duration-300", i < index ? "bg-emerald-800" : i === index ? "bg-emerald-400" : "bg-slate-800")} />
        ))}
      </div>
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-4">
        <div className="grid w-full max-w-3xl grid-cols-1 items-center gap-6 md:grid-cols-2">
          <RiskManagementPlayground visual={scene.visual} reduceMotion={reduceMotion} sceneKey={sceneKey} animate={playing} />
          <div className="border border-slate-700/60 bg-slate-900/40 p-4">
            <span className={cn(TERMINAL_TYPO.micro, "text-emerald-500")}>{scene.chapter}</span>
      ...
      <div className="flex shrink-0 flex-wrap items-center justify-center gap-2 border-t border-slate-800 px-3 py-2.5">
        ...
        <button type="button" onClick={togglePlay} className={cn(TERMINAL_TYPO.micro, "border px-2 py-1", playing ? "border-emerald-500/60 text-emerald-200" : "border-slate-700 text-slate-300")}>
        ...
        {isLast ? (
          <button type="button" onClick={findItLive} className={cn(TERMINAL_TYPO.micro, "flex items-center gap-1 border border-emerald-500/70 bg-emerald-950/40 px-2 py-1 text-emerald-200")}>
            FIND IT LIVE <Radio className="h-3 w-3" />
          </button>
        ) : (
          <button type="button" onClick={() => setIndex(index + 1)} className={cn(TERMINAL_TYPO.micro, "border border-emerald-700/50 px-2 py-1 text-emerald-300")}>
